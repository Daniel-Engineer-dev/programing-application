...
export default function EditGuidePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    const fetch = async () => {
        try {
            const snap = await getDoc(doc(db, "guides", resolvedParams.id));
            if (snap.exists()) {
  ...
  if (!data) return <div className="p-10 text-center text-red-500">Không tìm thấy hướng dẫn!</div>;

  return (
    <div className="p-6 min-h-screen">
      <GuideForm initialData={data} isEditing={true} />
    </div>
  );
}

...
export default function RichTextEditor({ value, onChange }: RichTextEditorProps) {
  ...
  const handleOl = () => {
    ...
    const next = (value || "").slice(0, start) + newLine + (value || "").slice(end);
    const delta = newLine.length - line.length;
    ...
    >
      ...
      {hoverTip === tip && (
        <span className="absolute left-1/2 top-[calc(100%+8px)] z-50 -translate-x-1/2 whitespace-nowrap rounded-md border border-slate-700 bg-slate-950 px-2 py-1 text-[11px] text-slate-100 shadow-lg">
          {TIP_TEXT[tip]}
          <span className="absolute left-1/2 top-[-6px] h-3 w-3 -translate-x-1/2 rotate-45 border-l border-t border-slate-700 bg-slate-950" />
        </span>
      )}
    </button>
  );

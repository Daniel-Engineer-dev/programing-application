...
export default function PathDetail() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        const res = await fetch(`/explore/api/explore/path/${id}`);
        if (!res.ok) {
            if (res.status === 404) {
                setError(true);
                return;
  ...
  if (error) {
    return (
      <div className="min-h-screen bg-slate-950 flex flex-col items-center justify-center text-center p-4">
          ...
          <Link href="/explore" className="px-6 py-3 bg-blue-600 hover:bg-blue-500 rounded-xl text-white font-bold transition-all">
              Quay lại Khám Phá
          </Link>
      </div>
    )
  ...
  const baseUrl = "/explore/explorePage";

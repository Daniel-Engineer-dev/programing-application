"use client";
import { markdownToHtml } from "@/components/Discussion/Markdown";
...
export default function DiscussionDetailPage() {
  ...
  const [repliesLastDocMap, setRepliesLastDocMap] = useState<
    Record<string, any>
  >({});
  ...
  const [commentVoteMap, setCommentVoteMap] = useState<
  ...
  // xoá modal (comment)
  const [confirmCommentDeleteOpen, setConfirmCommentDeleteOpen] =
    useState(false);
  ...
  const [reportCommentSuccessOpen, setReportCommentSuccessOpen] =
    useState(false);
    ...
    const run = async () => {
      ...
      try {
        ...
        if (voteSnap.exists()) {
          ...
          setCurrentVote(
            v.type === "like" || v.type === "dislike" ? v.type : null
          );
        } else {
          setCurrentVote(null);
        }
  ...
  const sortedParents = useMemo(() => {
    ...
    const score = (c: CommentItem) =>
      (c.likesCount ?? 0) - (c.dislikesCount ?? 0);
    const votes = (c: CommentItem) =>
      (c.likesCount ?? 0) + (c.dislikesCount ?? 0);
  ...
  // REPORT HELPERS
  // =========================
  const toggleReason = (arr: string[], reason: string) =>
    arr.includes(reason) ? arr.filter((x) => x !== reason) : [...arr, reason];

  const submitReportPost = async () => {
    if (!discussionId || !discussion) return;
    if (!user) return alert("Bạn cần đăng nhập để báo cáo.");
    if (reportPostSelected.length === 0)
      return alert("Vui lòng chọn ít nhất 1 lý do báo cáo.");
    if (reportPostSelected.includes("Khác") && !reportPostOther.trim())
      return alert("Vui lòng nhập nội dung cho mục 'Khác'.");

    try {
      setReportPostSubmitting(true);
      await addDoc(
        collection(db, "discussions", discussionId as string, "reports"),
        {
          reasons: reportPostSelected,
          details: reportPostSelected.includes("Khác")
            ? reportPostOther.trim()
            : "",
          reporter: {
            uid: user.uid,
            name: user.displayName || user.email || "Người dùng",
  ...
  const submitReportComment = async () => {
    ...
    if (!user) return alert("Bạn cần đăng nhập để báo cáo.");
    if (reportCommentSelected.length === 0)
      return alert("Vui lòng chọn ít nhất 1 lý do báo cáo.");
    if (reportCommentSelected.includes("Khác") && !reportCommentOther.trim())
      return alert("Vui lòng nhập nội dung cho mục 'Khác'.");
    ...
    try {
      ...
      await addDoc(
        ...
        ),
        {
          ...
          details: reportCommentSelected.includes("Khác")
            ? reportCommentOther.trim()
            : "",
          reporter: {
            uid: user.uid,
            name: user.displayName || user.email || "Người dùng",
  ...
  const commentScore = (c: CommentItem) =>
    (c.likesCount ?? 0) - (c.dislikesCount ?? 0);
  const commentVotes = (c: CommentItem) =>
    (c.likesCount ?? 0) + (c.dislikesCount ?? 0);

  const renderCommentCard = (
    c: CommentItem,
    isReply: boolean,
    parentIdForReplyList?: string
  ) => {
        ...
        <div className="flex items-start justify-between gap-4">
          <div className="flex flex-col">
            <div className="flex items-center gap-2 text-sm text-slate-300">
              ...
              {c.updatedAt && (
                <span className="text-slate-500">(đã chỉnh sửa)</span>
              )}
            </div>
            ...
            {editingId === c.id ? (
              ...
              <div
                className="mt-3 text-sm text-slate-200 leading-relaxed"
                dangerouslySetInnerHTML={{
                  __html: linkifyTextToHtml(c.content),
                }}
              />
            )}
          </div>
  ...
  return (
    <div className="relative min-h-screen bg-slate-950 text-slate-200">
      <main className="relative z-10 mx-auto max-w-4xl space-y-8 px-4 py-8 sm:px-6 lg:px-8">
        ...
        {/* Header Card */}
        <header className="rounded-2xl border border-slate-800 bg-slate-900 p-6 shadow-sm sm:p-8">
                  ...
                  <div className="absolute right-0 mt-2 w-56 rounded-xl border border-slate-700 bg-slate-900 p-2 shadow-xl z-10">
                    <button
                      ...
                      onClick={() => {
                        ...
                        router.push(`/discuss/${discussion.id}/edit`);
                      }}
        ...
        <section className="space-y-6">
          <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
            <h2 className="flex items-center gap-2 text-xl font-semibold text-white">
              ...
              <span>Bình luận</span>
              <span className="rounded-md bg-slate-800 border border-slate-700 px-2 py-0.5 text-xs text-slate-300">{discussion.repliesCount}</span>
            </h2>
            ...
            <div className="relative">
              <button
                ...
                className="inline-flex items-center gap-2 rounded-xl border border-slate-800 bg-slate-900 px-4 py-2.5 text-sm text-slate-200 hover:bg-slate-800 transition-colors"
              >
              ...
              {sortOpen && (
                <div className="absolute right-0 mt-2 w-64 rounded-2xl border border-slate-800 bg-slate-900 p-2 shadow-lg z-10">
                  {[
                    { key: "BEST" as const, label: "Hay nhất" },
                    {
                      key: "MOST_VOTES" as const,
                      label: "Nhiều lượt bình chọn nhất",
                    },
          ...
          <div className="rounded-2xl border border-slate-800 bg-slate-900 p-5 shadow-sm">
            <textarea
              ...
              rows={4}
              className="w-full resize-none rounded-xl border border-slate-800 bg-slate-950 px-4 py-3 text-sm text-slate-200 placeholder-slate-500 outline-none transition-all focus:border-blue-500"
            ...
            <div className="mt-4 flex justify-end">
              <button
                ...
                className="rounded-xl bg-emerald-600 px-6 py-2.5 text-sm font-semibold text-white transition-colors hover:bg-emerald-500 disabled:cursor-not-allowed disabled:opacity-50"
          ...
          <div className="flex items-center justify-center gap-2 pt-2">
            ...
            <span className="rounded-lg border border-slate-700 bg-slate-800 px-4 py-2 text-sm text-slate-200">
              Trang {parentPage}
            </span>
        ...
        {confirmCommentDeleteOpen && (
          <div className="fixed inset-0 z-9999 flex items-center justify-center bg-black/60">
            <div className="w-full max-w-md rounded-xl border border-slate-700 bg-slate-900 p-6 shadow-2xl">
              ...
              <p className="mb-6 text-sm text-slate-300">
                Bạn có chắc chắn muốn xoá bình luận này không? Hành động này
                không thể hoàn tác.
              </p>
              <div className="flex justify-end gap-3">
                <button
